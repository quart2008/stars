HA$PBExportHeader$appeondom_builder.sru
forward
global type appeondom_builder from nonvisualobject
end type
end forward

global type appeondom_builder from nonvisualobject
end type
global appeondom_builder appeondom_builder
...
//appeondom_document idom_document
appeondom_doctype idom_document
end variables

forward prototypes
public function appeondom_document buildfromstring (string as_xml)
public function appeondom_document buildfromfile (string as_file)
public function appeondom_document buildfromdatastore (datastore ads_data)
public function appeondom_document buildfromdatastore (datastore ads_data, boolean abl_apb)
end prototypes

public function appeondom_document buildfromstring (string as_xml);boolean lb_success
string ls_rtn
...
public function appeondom_document buildfromdatastore (datastore ads_data);integer li_Return
appeondom_document ldom_document
string ls_Xml = "c:\$appeondom_data.xml"
...
public function appeondom_document buildfromdatastore (datastore ads_data, boolean abl_apb);string ls_Xml
integer li_Return
appeondom_document		ldom_document
ldom_document = Create appeondom_Document
...
appeondom_ELEMENT		Element_Root,Element_datarow,Element_column
Element_Root = ldom_document.GetRootElement()

for i = 1 to ads_data.rowcount()
			//Add the data_row Element
			if not isvalid(Element_datarow) then Element_datarow = CREATE appeondom_ELEMENT
			if not isvalid(Element_column) then Element_column = CREATE appeondom_ELEMENT						
...
on appeondom_builder.create
call super::create
TriggerEvent( this, "constructor" )
end on

on appeondom_builder.destroy
...
idom_document = create appeondom_doctype
