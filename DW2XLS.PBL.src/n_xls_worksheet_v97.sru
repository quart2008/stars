...
public function integer of_write (long ai_row, long ai_col, double aa_value, long ai_format);int li_ret = 1
uint li_record =   515  //0x0203 Record identifier
uint li_length =   14   //0x000E Number of bytes to follow
uint li_xf
blob lb_header
...
public function integer of_write (long ai_row, long ai_col, string aa_value, long ai_format);return of_write(ai_row, ai_col, invo_sub.to_unicode(invo_sub.of_str2xls(aa_value)), ai_format)
end function
...
if isNull(aa_value) then aa_value = blob('')
//as_str = invo_sub.of_str2xls(as_str)
//li_strlen = len(as_str)
...
if li_ret = 1 then
   ll_sst_index = invo_workbook.event ue_add_unicode(aa_value)
end if
...
public subroutine of_set_merge_range (long ai_format);
invo_workbook.of_get_format(ai_format).of_set_merge_range()
end subroutine
